...
export default function PractitionersPage() {
  ...
  function openCreate() {
    setEditing(null);
    setForm({ first_name: "", last_name: "", document: "", email: "", phone: "", role: "practicante", study: "", hire_date: new Date().toISOString().split("T")[0], status: "active" });
    setShowModal(true);
  }

  function openEdit(p: Practitioner) {
    setEditing(p);
    setForm({ first_name: p.first_name, last_name: p.last_name, document: p.document || "", email: p.email || "", phone: p.phone || "", role: p.role, study: p.study || "", hire_date: p.hire_date, status: p.status });
    setShowModal(true);
  }
  ...
  if (loading) {
    return <div className="flex h-64 items-center justify-center"><div className="flex flex-col items-center gap-3"><div className="w-10 h-10 border-4 border-primary border-t-transparent rounded-full animate-spin" /><span className="text-sm font-medium text-gray-500 dark:text-gray-400">Cargando practicantes...</span></div></div>;
  }
    ...
    <div className="space-y-6 animate-fade-in">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-11 h-11 rounded-xl gradient-warm flex items-center justify-center shadow-md"><BriefcaseIcon className="w-6 h-6 text-white" /></div>
          <div><h1 className="text-2xl font-bold text-gray-900 dark:text-white tracking-tight">Practicantes</h1><p className="text-sm text-gray-500 dark:text-gray-400">{practitioners.length} registros</p></div>
        </div>
          ...
          {filtered.map((p) => (
            <div key={p.id} className="bg-white dark:bg-[#1a2438] rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm group">
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  <div className="w-10 h-10 rounded-xl gradient-warm flex items-center justify-center text-white text-sm font-bold shadow-sm">{p.first_name.charAt(0)}{p.last_name.charAt(0)}</div>
                  <div><h3 className="font-bold text-gray-900 dark:text-white text-[15px]">{p.first_name} {p.last_name}</h3><p className="text-[12px] text-gray-500 dark:text-gray-400">{p.role}</p></div>
                </div>
                <div className="flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                  <button onClick={() => openEdit(p)} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-primary/10 hover:text-primary transition-colors"><PencilIcon className="w-4 h-4" /></button>
                  <button onClick={() => setDeleteConfirm(p.id)} className="w-8 h-8 rounded-lg flex items-center justify-center text-gray-400 hover:bg-red-50 hover:text-red-500 dark:hover:bg-red-900/20 transition-colors"><TrashIcon className="w-4 h-4" /></button>
                </div>
              </div>
              <div className="space-y-2">
                <div className="flex items-center gap-2 text-[13px]"><span className="text-gray-500 dark:text-gray-400">Correo:</span><span className="font-medium text-gray-900 dark:text-white truncate">{p.email || "Sin correo"}</span></div>
                <div className="flex items-center gap-2 text-[13px]"><span className="text-gray-500 dark:text-gray-400">Estudios:</span><span className="font-medium text-gray-900 dark:text-white">{p.study || "No especificado"}</span></div>
              </div>
              <div className="mt-4 pt-3 border-t border-gray-100 dark:border-gray-800">
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-semibold ${p.status === "active" ? "bg-emerald-50 text-emerald-600 dark:bg-emerald-900/20 dark:text-emerald-400" : "bg-gray-100 text-gray-500 dark:bg-gray-800 dark:text-gray-400"}`}><span className={`w-2 h-2 rounded-full ${p.status === "active" ? "bg-emerald-500" : "bg-gray-400"}`} />{p.status === "active" ? "Activo" : "Inactivo"}</span>
              </div>
            </div>
          ))}
        </div>
      ) : (
        <div className="bg-white dark:bg-[#1a2438] rounded-2xl p-12 border border-gray-100 dark:border-gray-800 shadow-sm text-center"><BriefcaseIcon className="w-12 h-12 mx-auto text-gray-400 dark:text-gray-500 mb-3" /><p className="text-gray-700 dark:text-gray-300 font-medium">No se encontraron practicantes</p><p className="text-sm text-gray-500 dark:text-gray-400 mt-1">{search ? "Intenta con otro termino" : "Registra el primer practicante"}</p></div>
      )}
      ...
      <Modal open={!!deleteConfirm} onClose={() => setDeleteConfirm(null)} title="Confirmar eliminacion">
        <p className="text-gray-600 dark:text-gray-300">Estas seguro de eliminar este practicante?</p>
