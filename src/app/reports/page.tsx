...
export default function ReportsPage() {
  ...
  async function loadReport() {
    ...
    const [childrenSnap, groupsSnap, attendanceSnap] = await Promise.all([
  ...
  function handleExportExcel() {
    exportToExcel(reportData.map((r) => ({ "Nombre": r.childName, "Grupo": r.groupName, "Presentes": r.totalPresent, "Ausentes": r.totalAbsent, "% Asistencia": r.percentage })), `reporte_asistencia_${filter}`);
  }
  function handleExportPDF() {
    exportToPDF("Reporte de Asistencia", ["Nombre", "Grupo", "Presentes", "Ausentes", "% Asistencia"], reportData.map((r) => [r.childName, r.groupName, String(r.totalPresent), String(r.totalAbsent), `${r.percentage}%`]), `reporte_asistencia_${filter}`);
  }
  ...
  const pieData = [{ name: "Presentes", value: totalPresent || 0 }, { name: "Ausentes", value: totalAbsent || 0 }];
  ...
  return (
    <div className="space-y-6 animate-fade-in">
      ...
      <div className="grid gap-4 lg:grid-cols-3">
        <div className="bg-white dark:bg-[#1a2438] rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm text-center"><p className="text-3xl font-bold text-gray-900 dark:text-white">{reportData.length}</p><p className="text-[12px] font-medium text-gray-500 dark:text-gray-400">Ninos evaluados</p></div>
        <div className="bg-white dark:bg-[#1a2438] rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm text-center"><p className="text-3xl font-bold text-emerald-500">{totalPresent}</p><p className="text-[12px] font-medium text-gray-500 dark:text-gray-400">Total asistencias</p></div>
        <div className="bg-white dark:bg-[#1a2438] rounded-2xl p-5 border border-gray-100 dark:border-gray-800 shadow-sm text-center"><p className="text-3xl font-bold text-red-500">{totalAbsent}</p><p className="text-[12px] font-medium text-gray-500 dark:text-gray-400">Total ausencias</p></div>
      </div>
      ...
      <div className="bg-white dark:bg-[#1a2438] rounded-2xl border border-gray-100 dark:border-gray-800 shadow-sm overflow-hidden">
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            <thead><tr className="border-b border-gray-100 dark:border-gray-800"><th className="text-left px-5 py-3.5 font-semibold text-gray-500 dark:text-gray-400">Nombre</th><th className="text-left px-5 py-3.5 font-semibold text-gray-500 dark:text-gray-400">Grupo</th><th className="text-center px-5 py-3.5 font-semibold text-gray-500 dark:text-gray-400">Presentes</th><th className="text-center px-5 py-3.5 font-semibold text-gray-500 dark:text-gray-400">Ausentes</th><th className="text-center px-5 py-3.5 font-semibold text-gray-500 dark:text-gray-400">% Asistencia</th></tr></thead>
            <tbody>
              {reportData.map((r, i) => (
        ...
        {reportData.length === 0 && <div className="py-12 text-center text-gray-500 dark:text-gray-400">No hay datos para mostrar en este periodo.</div>}
      </div>
    </div>
  );
}
